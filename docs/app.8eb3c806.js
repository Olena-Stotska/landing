...
function main() {
  ...
  return getArticles().then(renderArticles).catch(function (error) {
    console.error(error);
  });
}
...
function renderArticles(articles) {
  var template = document.getElementById('post-container');
  var main = document.getElementById('container');

  articles.forEach(function (article) {
    var articleNode = template.content.cloneNode(true);
    main.appendChild(render(articleNode, article));
  });
}

function render(element, obj) {
  var elements = Array.from(element.querySelectorAll('[data-bind]'));

  elements.forEach(function (el) {
    var bindings = el.getAttribute('data-bind').split(',');

    bindings.forEach(function (binding) {
      var _binding$split = binding.split(':'),
          _binding$split2 = _slicedToArray(_binding$split, 2),
          field = _binding$split2[0],
          attr = _binding$split2[1];
...
function playVideo() {
  document.addEventListener('click', function (event) {
    ...
    if (target.name === 'play') {
      var video = target.parentNode.querySelector('video');
